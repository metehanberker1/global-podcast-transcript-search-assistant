...
def ingest_feed(
    ...
    *,
    ...
    def _local_ingest() -> IngestResult:
        started = time.perf_counter()

        existing_lookup = registry.find_by_input_url(feed_url)
        if (
            existing_lookup.entry is not None
            and existing_lookup.entry.last_indexed_at is not None
        ):
            # Reuse existing indexed data when this feed is already ingested.
        ...
        if not episodes:
            raise RuntimeError("RSS feed contains no episode entries.")

        # Build stable chunk IDs and per-chunk metadata for storage/retrieval.
        chunk_ids: list[str] = []
        documents: list[str] = []
        ...
        # Refresh cache immediately so subsequent searches use warm handles.
    ...
    routed = router.route_action(feed_id=feed_id, action=_local_ingest)
