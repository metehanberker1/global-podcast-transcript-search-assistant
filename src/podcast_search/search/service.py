...
def search(conn: Any, *, query: str, k: int) -> list[SearchHit]:
    ...
    # Query scope is the last successfully ingested feed.
    ...
    def _local_search() -> list[SearchHit]:
        started = time.perf_counter()

        # Planner output is used as embedding input text.
        ...
        hits = query_collection(
            ...
            k=k,
            persist_dir=index_handle.persist_dir,
        )
        from podcast_search.metrics.service import record_chroma_query_duration_ms_last

        record_chroma_query_duration_ms_last(
            int((time.perf_counter() - started_chroma) * 1000)
        )
    ...
    routed = router.route_action(feed_id=entry.feed_id, action=_local_search)
