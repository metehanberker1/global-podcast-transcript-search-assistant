...
class FeedRegistry:
    ...
    def load(self) -> dict[str, FeedRegistryEntry]:
        ...
        for feed_id, entry in feeds_raw.items():
            try:
                ...
                # If entry can't be parsed, skip it (Phase 2 can re-ingest).
                continue
        return out
    ...
    def find_by_input_url(self, feed_url: str) -> RegistryLookup:
        ...
        # Direct match on normalized_url first.
    ...
    def upsert_entry_after_ingest(
        ...
        *,
        ...
        # Update related URL set: always include the user input and parsed alternates.
