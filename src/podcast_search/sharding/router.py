...
class Router:
    """Deterministic router for feed-scoped sharding (Phase 6 MVP).

    For MVP we compute ownership via consistent hashing and execute the local
    action for now, but the selection logic is stable and testable.
    """
    ...
    def route_action(self, *, feed_id: str, action: Callable[[], Any], **kwargs: Any) -> RouteResult:
        ...
        if cb is not None:
            return RouteResult(owner=owner, result=cb(feed_id=feed_id, owner=owner, **kwargs))

        # Fallback: try local callback.
        local_cb = self._node_callbacks.get(self._local_node_id)
        if local_cb is not None:
            return RouteResult(owner=owner, result=local_cb(feed_id=feed_id, owner=owner, **kwargs))
